...
- **γ·보상 정규화 (§7-4, 코드로 확증):** 환경 보상이 정규화 안 됨(`winning_reward=+5000`, `reward_range=(-inf, inf)`)이고 에이전트도 raw 보상 사용(안정화는 Huber + grad clamp±1뿐, 보상 레벨 아님). → **`γ=0.015`는 근시안이라기보다 정규화 안 한 큰 보상을 안정화하려는 회피책.** γ만 0.9로 올리면 **발산**(CPU 실측: 학습 후에도 에피소드가 조기종료 못 하고 iter 상한까지 flailing). 즉 README의 "장기 의존성" 프레이밍과 달리 **핵심 가설(순환 메모리가 장기 신용할당에 유리)을 검증할 무대(고 γ)가 애초에 없었음.** → `agent_dql/agent_drqn`에 `reward_scale`, `fair_compare.py`에 `REWARD_SCALE`/`GAMMA`/`OUT_SUFFIX` 옵션 추가.
  - **정규화(reward_scale=0.01: 보상÷100)+γ=0.9 재실험 결과(부분, CPU 25ep/4000it):** defender owned이 **오히려 하락** — DQN ~2.9/12(γ=0.015의 4.8 대비↓), DRQN seed0 2.0(7.5 대비↓). 게다가 매우 느림(run당 22~135분, 에피소드가 조기종료 못 함). DRQN 전 seed 완주 전 CPU 9시간 경과해 중단.
  - **해석:** γ=0.9는 정규화해도 **이 학습예산(25ep)에선 더 나쁨** — 고 γ는 지연보상 신용을 역전파하는 데 훨씬 많은 에피소드가 필요한데 CPU에선 비현실적. 즉 "장기 무대(고 γ)에서 순환이 유리한가"는 *강한 정규화 + 고 γ + 대량 에피소드 + GPU* 가 갖춰져야 제대로 검증 가능 — **현 여건에선 미결(open)**. (§7-4 우려는 확증하되, 빠른 개선책은 없음을 실측으로 확인.)
